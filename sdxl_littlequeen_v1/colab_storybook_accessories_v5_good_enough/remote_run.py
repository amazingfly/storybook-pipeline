#!/usr/bin/env python3
"""Run the conservative Little Queen accessory workflow and retain grip variants."""
...
ROOT = Path("/content/lq_storybook_accessories_v5_good_enough")
BUNDLE_PREFIX = Path("/content/storybook_accessories_v5_good_enough_bundle.tar.gz.part_")
BASES_PREFIX = Path("/content/storybook_accessories_v5_good_enough_bases.tar.gz.part_")
...
ARCHIVE = Path("/content/lq_storybook_accessories_v5_good_enough_results.tar.gz")
...
def main() -> int:
    ...
    summary = {
        "name": "littlequeen_storybook_accessories_v5_good_enough",
        "status": "running",
    ...
    positive_template = (
        "full body, solo, lqxl Little Queen, lqmoonfit, bare head, no jewelry, upright front "
        "three-quarter pose, exactly two arms, one visible waist-height hand closed around a "
        "thin vertical wooden rod outside her body, other hand visible, head hands shoes "
        "framed, {scene}, anime storybook"
    )
    negative = (
        "crown, tiara, hat, hair ornament, ornate wand, staff, sword, weapon, floating rod, "
        "angled rod, open palm, raised hand, reaching, waving, walking, dancing, action pose, "
        "jewelry, cropped head, cropped hands, cropped feet, hidden hands, close-up, two people, "
        "extra limbs, malformed hands, photo, text, watermark"
    )
    ...
    try:
        ...
        summary["status"] = (
            ...
            if len(review_pool) >= target
            else "insufficient_geometry_candidates"
        )
        summary["finished_at"] = datetime.now(timezone.utc).isoformat()
        write_summary(summary)
        build_archive()
        print(
            f"Conservative run complete: {len(review_pool)} geometry candidates "
            f"awaiting visual review; 0 automatically selected.",
            flush=True,
        )
        return 0
    finally:
