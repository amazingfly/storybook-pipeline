...
SESSION="${COLAB_SESSION:-lq-storybook-accessories-v5-good-enough}"
GPU="${COLAB_GPU:-T4}"
BUNDLE="${ROOT}/storybook_accessories_v5_good_enough/colab/storybook_accessories_v5_good_enough_bundle.tar.gz"
RUN_STAMP="$(date +%Y%m%d_%H%M%S)"
RESULTS="${COLAB_RESULTS:-${ROOT}/outputs/storybook_accessories_v5_good_enough/colab_run_${RUN_STAMP}}"
SUCCESS=0
ALLOCATED=0
...
python "${ROOT}/scripts/build_storybook_accessories_v5_good_enough_bundle.py"
...
ALLOCATED=1
upload_chunked "${BUNDLE}" "storybook_accessories_v5_good_enough_bundle.tar.gz.part_"
if [[ -n "${COLAB_REUSE_BASES:-}" ]]; then
  ...
  upload_chunked "${BASE_ARCHIVE}" "storybook_accessories_v5_good_enough_bases.tar.gz.part_"
  rm -f "${BASE_ARCHIVE}"
fi
...
colab upload -s "${SESSION}" \
  "${ROOT}/colab_storybook_accessories_v5_good_enough/remote_run.py" \
  /content/remote_storybook_accessories_v5_good_enough.py
if ! colab exec -s "${SESSION}" \
  -f "${ROOT}/colab_storybook_accessories_v5_good_enough/launch.py" \
  --timeout 21600; then
...
"${COLAB_PYTHON}" "${ROOT}/scripts/recover_colab_artifact.py" \
  --session "${SESSION}" \
  /content/lq_storybook_accessories_v5_good_enough_results.tar.gz \
...
SUCCESS=1
printf 'Downloaded conservative V5 storybook run to %s\n' "${RESULTS}"
